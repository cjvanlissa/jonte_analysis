do_brma <- function(dat, yvar = "yi", cluster_var = "ArticleID", ...){
  browser()
  tuning_pars <- expand.grid(
    relevant_pars = 3#c(1, 3, 5, 7, 9)
  )

  cv_rmses <- sapply(1:nrow(tuning_pars), function(i){
    sapply(dat$fold_rownums, function(thisfold){
      fit_cv <- pema::brma(
        formula = as.formula(paste0(yvar, "~", paste0(setdiff(names(dat$train), c(yvar, "vi", cluster_var)), collapse = " + "))),
      ...
      mean((dat$train[[yvar]][thisfold] - preds)^2)
    })
  })
